...
// Find where "let themeBgClass = " is
const bgClassStart = lines.findIndex(l => l.includes('let themeBgClass = '));
const returnStart = lines.findIndex((l, i) => i > bgClassStart && l.trim() === 'return (');

const newVariablesAndReturn = `  // Determinar classes visuais conforme o tema selecionado
...
// Replace from 'let themeBgClass...' to the END of the file (or end of DashboardShell function)
const newLines = [
  ...lines.slice(0, bgClassStart),
  newVariablesAndReturn,
  '}' // Fecha a function DashboardShell
];

fs.writeFileSync(shellPath, newLines.join('\\n'), 'utf8');
console.log('DashboardShell.tsx overwritten perfectly.');
