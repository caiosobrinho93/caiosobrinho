...
const newAside = `{/* 1. SIDEBAR DESKTOP - Retrátil e Flutuante (Glassmorphism) */}
      <motion.aside
        ...
        className="hidden md:flex flex-col h-[calc(100vh-32px)] my-4 ml-4 z-40 absolute left-0"
        style={{ transitionProperty: "none" }}
      >
        <div className="glass-panel w-full h-full flex flex-col relative !overflow-visible">
          {/* Header */}
              ...
              <AnimatePresence>
                {!isSidebarCollapsed && (
                  <motion.div
                    ...
                    transition={{ duration: 0.2, delay: isSidebarCollapsed ? 0 : 0.2 }}
                    ...
                    <AnimatePresence>
                      {!isSidebarCollapsed && (
                        <motion.span
                          ...
                          exit={{ opacity: 0, width: 0 }}
                          transition={{ duration: 0.2, delay: isSidebarCollapsed ? 0 : 0.2 }}
                          className="whitespace-nowrap font-medium text-sm tracking-wide overflow-hidden"
                        >
                          {item.name}
                        </motion.span>
                      )}
        ...
        {/* Botão de Toggle Manual fora do glass-panel para não ser cortado */}
        <button 
          onClick={() => setIsSidebarCollapsed(v => !v)}
          className="absolute -right-3 top-12 w-8 h-8 rounded-full bg-primary/80 backdrop-blur-md border border-white/20 text-white flex items-center justify-center shadow-[0_0_15px_rgba(94,92,230,0.5)] z-50 cursor-pointer hover:scale-110 hover:bg-primary transition-all"
        >
