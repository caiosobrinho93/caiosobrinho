...
const startIdx = lines.findIndex((l, i) => i > 800 && l.trim() === 'return (');
...
// Now standardize specific old classes inside page.tsx helpers
let finalContent = fs.readFileSync(pagePath, 'utf8');
finalContent = finalContent.replace(/className="[^"]*neon-glow-card[^"]*"/g, 'className="flex flex-col h-full"');
finalContent = finalContent.replace(/bg-card\/40/g, 'bg-transparent');
finalContent = finalContent.replace(/bg-black\/40/g, 'bg-white/5');
finalContent = finalContent.replace(/border-border/g, 'border-white/10');
finalContent = finalContent.replace(/border-primary\/30/g, 'border-white/20');
finalContent = finalContent.replace(/bg-muted\/20/g, 'bg-white/5');
finalContent = finalContent.replace(/text-muted-foreground/g, 'text-white/60');
finalContent = finalContent.replace(/text-foreground/g, 'text-white/90');
fs.writeFileSync(pagePath, finalContent, 'utf8');
console.log('page.tsx overwritten perfectly.');
